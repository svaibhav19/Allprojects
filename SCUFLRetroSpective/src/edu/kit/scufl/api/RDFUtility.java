...
public class RDFUtility {
	...
	public Resource createProcessExec(String processId, String processTitle, String activityId, String startTime,
			String endTime, String completed) {
		Resource processExecResource = model.createResource(ns + "processExec_" + processId);
		// short uri Resource
		// process=model.createResource([http://]"http://"+processId);
		...
		// Property
		// subProcessProperty=model.createProperty(provone+"hasSubProcess");
		// model.add(workflow,subProcessProperty,process);
		return processExecResource;
	}
	// public Resource createWorkflowExec(String workflowId, String
	// workflowTitle, String startTime, String endTime, String completed) {
	// Resource processExecResource = model.createResource(ns + "processExec_" +
	// workflowId);
	// //short uri Resource
	// process=model.createResource([http://]"http://"+processId);
	// processExecResource.addProperty(RDF.type, model.createResource(provone +
	// "ProcessExec"));
	...
	public Resource createSeqCtrlLink(Resource process1, Resource process2, String sequenceCLID) {
		...
		// short uri Resource
		// seqCtrlLink=model.createResource([http://]"http://"+p1Id+"_"+p2Id+"CL");
